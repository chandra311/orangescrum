            ...
            if (file.name != "error")
            {
                if (file.message == "success")
                {
                    ...
                    if (parseInt(file.sizeinkb) <= parseInt(allowedSize)) {
                        ...
                        return $('<tr><td style="color:#0683B8;" valign="top"><div id="' + csAtId + 'jquerydiv' + i + '"><input type="checkbox" checked onClick="return removeFile(\'' + csAtId + 'jqueryfile' + i + '\',\'' + csAtId + 'jquerydiv' + i + '\');" style="cursor:pointer;"/>&nbsp;&nbsp;<a href="' + HTTP_ROOT + 'easycases/download/' + fname[0] + '/loading" style="text-decoration:underline;position:relative;top:-2px;">' + file.name + ' (' + filesize + ')</a><input type="hidden" name="data[Easycase][name][]" id="' + csAtId + 'jqueryfile' + i + '" value="' + file.filename + '"/><\/div><\/td><\/tr>');
                    }
                    else {
