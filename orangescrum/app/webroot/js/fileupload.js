			...
			if(file.name != "error")
			{
				if(file.message == "success")
				{
					...
					if(parseInt(file.sizeinkb) <= parseInt(allowedSize)) {
						...
						return $('<tr><td style="color:#0683B8;" valign="top"><div id="jquerydiv'+i+'"><input type="checkbox" checked onClick="return removeFile(\'jqueryfile'+i+'\',\'jquerydiv'+i+'\','+sizeinMb+');" style="cursor:pointer;"/>&nbsp;&nbsp;<a href="'+pageurl+'easycases/download/'+fname[0]+'/loading" style="text-decoration:underline;position:relative;top:-2px;">'+file.name+' ('+filesize+')</a><input type="hidden" class="ajxfileupload" name="data[Easycase][name][]" id="jqueryfile'+i+'" value="'+file.filename+'"/><\/div><\/td><\/tr>');
						//alert(i);
					}
					else {
