...
//		    $name = trim(substr($value['User']['name'],0,7))."...";
	    ?>
		<td align="left">
		    <label class="userLbl ellipsis-view maxWidth80" title="<?php echo $value['User']['name'];?>"><input type="checkbox" name="data[Project][user][]" class="prj_users" onclick="checkUncheckAll(0);" style="cursor: pointer;" id="userId_<?php echo $value['User']['uniq_id'];?>" value="<?php echo $value['User']['uniq_id'];?>" <?php if(count($uids) > 0 && (in_array($value['User']['id'],$uids))){ ?>checked="checked" <?php }elseif(count($uids) == 0){?>checked="checked" <?php }?> />&nbsp;&nbsp;<?php echo $this->Format->mb_ucfirst($name);?></label>
		</td>
	    <?php if(($cnt%3) == 0){ ?>
	    </tr>
	     <?php }?>
