...
class Project extends AppModel {
    ...
    function getCompAllPrjIds() {
        $this->recursive = -1;
        $allprj = $this->find('list', array('conditions' => array('Project.company_id' => SES_COMP, 'Project.isactive' => 1), 'fields' => array('Project.id')));
        return $allprj;
    }
    ...
    /**
     * @method deleteprojects Deleting project and all associated data from project

     * @return array 
     */
    function deleteprojects($projuid, $comp_id = null) {
        $this->recursive = -1;
        $cmp_id = ($comp_id) ? $comp_id : SES_COMP;

        $proj = $this->find('first', array('conditions' => array('Project.uniq_id' => $projuid, 'Project.company_id' => $cmp_id)));
        if ($proj) {
            ...
            $case_files_list = $casefile_cls->find('list', array('conditions' => array('company_id' => $cmp_id, 'downloadurl IS NULL', 'project_id' => $prjid), 'fields' => array('id', 'file')));
            ...
            if ($this->delete($prjid, false)) {
                $arr['succ'] = 1;
                $arr['msg'] = __('Project deleted successfully', true);
            }
        } else {
            $arr['error'] = 1;
            $arr['msg'] = __('Oops! No project found with the given id.', true);
        }
        return $arr;
    }
    ...
    function getProjectId($name) {
        ...
        $project = $this->find('first', array('fields' => array('id'), 'conditions' => array('Project.name LIKE ' => trim(strtolower($name)))));
        $pro_id = !empty($project['Project']['id']) ? $project['Project']['id'] : '';
        return $pro_id;
    }

    function getPrjIdFromUniqid($uniq_id = null) {
        $this->recursive = -1;
        if ($uniq_id) {
            $prj_id = $this->find('first', array('conditions' => array('Project.uniq_id' => $uniq_id, 'Project.isactive' => 1), 'fields' => array('Project.id')));
            return array($prj_id['Project']['id']);
        } else {
            return 0;
        }
    }
