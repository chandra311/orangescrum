...
class ProjectsController extends AppController {
    ...
    function manage($projtype = NULL) {
        ...
        if (SES_TYPE == 3) {
            //$query .= " AND Project.user_id=" . $this->Auth->user('id');
            if ($pjname) {
                $prjAllArr = $this->Project->query("SELECT SQL_CALC_FOUND_ROWS Project.id,uniq_id,name,Project.user_id,project_type,short_name" . $wfl_cndn . ",Project.isactive,dt_updated,dt_created,(select count(easycases.id) as tot from easycases where easycases.project_id=Project.id and easycases.istype='1' and easycases.isactive='1') as totalcase,(select ROUND(SUM(easycases.hours), 1) as hours from easycases where easycases.project_id=Project.id and easycases.reply_type='0' and easycases.isactive='1') as totalhours,(select count(company_users.id) as tot from company_users, project_users where project_users.user_id = company_users.user_id and project_users.company_id = company_users.company_id and company_users.is_active = 1
	and project_users.project_id = Project.id) as totusers,(SELECT SUM(case_files.file_size) AS file_size FROM case_files WHERE case_files.project_id=Project.id) AS storage_used FROM projects AS Project WHERE Project.name!='' " . $query . " and name LIKE '%" . addslashes($pjname) . "%' ORDER BY dt_created DESC $limit ");
            } else {
                $prjAllArr = $this->Project->query("SELECT SQL_CALC_FOUND_ROWS Project.id,uniq_id,name,Project.user_id,project_type,short_name" . $wfl_cndn . ",Project.isactive,dt_updated,dt_created,(select count(easycases.id) as tot from easycases where easycases.project_id=Project.id and easycases.istype='1' and easycases.isactive='1') as totalcase,(select ROUND(SUM(easycases.hours), 1) as hours from easycases where easycases.project_id=Project.id and easycases.reply_type='0' and easycases.isactive='1') as totalhours,(select count(company_users.id) as tot from company_users, project_users where project_users.user_id = company_users.user_id and project_users.company_id = company_users.company_id and company_users.is_active = 1
	and project_users.project_id = Project.id) as totusers,(SELECT SUM(case_files.file_size) AS file_size FROM case_files WHERE case_files.project_id=Project.id) AS storage_used FROM projects AS Project WHERE Project.name!='' " . $query . " ORDER BY dt_created DESC $limit");
            }
        } else {
            if ($pjname) {
                $prjAllArr = $this->Project->query("SELECT SQL_CALC_FOUND_ROWS  id,uniq_id,name,user_id,project_type,short_name" . $wfl_cndn . ",isactive,dt_updated,dt_created,(select count(easycases.id) as tot from easycases where easycases.project_id=Project.id and easycases.istype='1' and easycases.isactive='1') as totalcase,(select ROUND(SUM(easycases.hours), 1) as hours from easycases where easycases.project_id=Project.id and easycases.reply_type='0' and easycases.isactive='1') as totalhours,(select count(company_users.id) as tot from company_users, project_users where project_users.user_id = company_users.user_id and project_users.company_id = company_users.company_id and company_users.is_active = 1
	and project_users.project_id = Project.id) as totusers,(SELECT SUM(case_files.file_size) AS file_size  FROM case_files WHERE case_files.project_id=Project.id) AS storage_used FROM projects AS Project WHERE name!='' " . $query . " and name LIKE '%" . addslashes($pjname) . "%' ORDER BY dt_created DESC $limit ");
            } else {
                $prjAllArr = $this->Project->query("SELECT SQL_CALC_FOUND_ROWS id,uniq_id,name,user_id,project_type,short_name" . $wfl_cndn . ",isactive,dt_updated,dt_created,(select count(easycases.id) as tot from easycases where easycases.project_id=Project.id and easycases.istype='1' and easycases.isactive='1') as totalcase,(select ROUND(SUM(easycases.hours), 1) as hours from easycases where easycases.project_id=Project.id and easycases.reply_type='0' and easycases.isactive='1') as totalhours,(select count(company_users.id) as tot from company_users, project_users where project_users.user_id = company_users.user_id and project_users.company_id = company_users.company_id and company_users.is_active = 1
	and project_users.project_id = Project.id) as totusers,(SELECT SUM(case_files.file_size) AS file_size  FROM case_files WHERE case_files.project_id=Project.id) AS storage_used FROM projects AS Project WHERE name!='' " . $query . " ORDER BY dt_created DESC $limit");
            }
        }
                        ...
                        if (!$i) {
                            ...
                            if (isset($value[$header_arr['title']]) && trim($value[$header_arr['title']])) {
                                foreach ($value as $k => $v) {
                                    ...
                                    // Parsing each data for error in data 
                                    else if (strtolower($fileds[$k]) == 'type' && $v) {
...
//        pr($this->request->data); exit;
        ...
        if (empty($validProject)) {
            $this->Session->write('ERROR', 'Oops! Error occured in importing task');
                ...
                while (($data = fgetcsv($handle, 1000, $separator)) !== FALSE) {
                    if (!$i) {
                        ...
                        if (isset($value[$header_arr['title']]) && trim($value[$header_arr['title']])) {
                            foreach ($value as $k => $v) {
                                $task_ass[strtolower($fileds[$k])] = $v;
                                if (strtolower($fileds[$k]) == 'project' && !empty($v)) {
...
//        exit;
        ...
        $this->set('total_task', $no_task);
        $this->set('proj_name', $pro_name);
            ...
            if (isset($_FILES['import_csv'])) {
                ...
                if (strtolower($ext) == 'csv') {
                    ...
                    if (($handle = fopen(CSV_PATH . "task_milstone/" . trim($file_name), "r")) !== FALSE) {
                        ...
                        foreach ($data as $key => $val) {
                            if (trim(strtolower($val)) == 'project') {
                                echo 1;
                                exit;
                            } else {
                                echo 2;
