...
//			}
    ...
    function ajax_activity() {
        ...
        if ($project_id == 'all') {
         ...
         $clt_sql = 1;
        if (defined('CR') && CR==1 && $this->Auth->user('is_client') == 1) {
            $clt_sql = "((Easycase.client_status = " . $this->Auth->user('is_client') . " AND Easycase.user_id = " . $this->Auth->user('id') . ") OR (Easycase.client_status != " . $this->Auth->user('is_client') . " AND Easycase.istype = 1))";
        }
